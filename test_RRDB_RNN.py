...
class DenseNet_TWOLAYER(nn.Module):
    def __init__(self, growth_rate=32, num_classes=5, times=2):
        ...
        # Recurrent block
        self.conv2 = nn.Conv2d(growth_rate * 2, growth_rate * 2, kernel_size=3, stride=1, padding=1, bias=False)
        
        # Dense block and transition
        self.dense_block1 = DenseBlock(growth_rate * 2, growth_rate, num_layers=1)
        self.transition = nn.Conv2d(growth_rate * 3, growth_rate * 2, kernel_size=1, stride=1, bias=False)
        self.times = times
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = model.state_dict()
                torch.save(best_model_wts, 'checkpoint_rrdb_rnn_withoutresidual_model.pth')
...
#torch.save(model.state_dict(), 'best_rrdb_rnn_withoutresidual_model.pth')
#model = train_model(model, dataloaders, criterion, optimizer, num_epochs=200)
...
state_dict = torch.load("/ssd4/ming/final/final/best_rrdb_rnn_model.pth")
